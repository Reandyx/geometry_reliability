...
topology_flag = 1;  % disconnected case
...
for i = 1:n
    ...
    if isfield(results_c(i), 'Pf_left')
        Pf_left(i) = results_c(i).Pf_left;
    else
        Pf_left(i) = NaN;
    end

    if isfield(results_c(i), 'Pf_right')
        Pf_right(i) = results_c(i).Pf_right;
    else
        Pf_right(i) = NaN;
    end

    if isfield(results_c(i), 'capture_ratio')
        capture(i) = results_c(i).capture_ratio;
    else
        capture(i) = NaN;
    end

    % --- CLASSIFICATION ---
    % Realistyczna klasyfikacja z danych, a nie stałych argumentów.
    % Logika:
    % - mały błąd FORM -> connected / FORM OK
    % - średni błąd -> transition
    % - duży błąd -> disconnected / FORM fails
    if err_form(i) < 0.1
        geom_class(i) = "CONNECTED";
    elseif err_form(i) < 0.5
        geom_class(i) = "TRANSITION";
    else
        geom_class(i) = "DISCONNECTED";
    end
...
legend('Left region', 'Right region', 'Location', 'best');
grid on;
...
for i = 1:n
    ...
    fprintf('c=%.2f | err_FORM=%.3f | class=%s | %s\n', ...
        c_vals(i), err_form(i), char(geom_class(i)), regime);
end
